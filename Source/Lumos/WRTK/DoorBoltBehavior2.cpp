...
void UDoorBoltBehavior2::BeginPlay()
{
	Super::BeginPlay();
	...
	for (auto& c : comps)
	{
		...
		if (sc->GetName() == ControlBoltName)
		{
			...
			ControlBolt = Cast<UStaticMeshComponent>(sc);
		...
		{
			foundOtherBolt = true;
			OtherBolt = Cast<UStaticMeshComponent>(sc);
...
// Called every frame
void UDoorBoltBehavior2::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
void UDoorBoltBehavior2::PostOnGrab(class UWRTK_InteractManager* im, class IGrabShapeInterface* shape)
{
...
void UDoorBoltBehavior2::PostOnGrabRelease(class UWRTK_InteractManager* im, class IGrabShapeInterface* shape)
{
